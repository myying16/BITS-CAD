...
class Extrude(object):
    ...
    def from_dict(all_stat, extrude_id, sketch_dim=256):
        ...
        operation = CAD_EXTRUDE_OPERATIONS.index(extrude_entity["operation"])
        extent_type = CAD_EXTENT_TYPE.index(extrude_entity["extent_type"])
        ...
        if operation == CAD_EXTRUDE_OPERATIONS.index("NewBodyFeatureOperation"):
            all_operations = [operation] + [CAD_EXTRUDE_OPERATIONS.index("JoinFeatureOperation")] * (n - 1)
        else:
    ...
    def from_vector(vec, is_numerical=False, n=256):
        ...
        assert vec[-1][0] == CAD_EXT_IDX and vec[0][0] == CAD_SOL_IDX
        profile_vec = np.concatenate([vec[:-1], CAD_EOS_VEC[np.newaxis]])
        profile = Profile.from_vector(profile_vec, is_numerical=is_numerical)
        ext_vec = vec[-1][-CAD_N_ARGS_EXT:]

        sket_pos = ext_vec[CAD_N_ARGS_PLANE:CAD_N_ARGS_PLANE + 3]
        sket_size = ext_vec[CAD_N_ARGS_PLANE + CAD_N_ARGS_TRANS - 1]
        sket_plane = CoordSystem.from_vector(np.concatenate([sket_pos, ext_vec[:CAD_N_ARGS_PLANE]]))
        ext_param = ext_vec[-CAD_N_ARGS_EXT_PARAM:]
    ...
    def to_vector(self, max_n_loops=6, max_len_loop=15, pad=True):
        ...
        ext_vec = np.array([CAD_EXT_IDX, *[PAD_VAL] * CAD_N_ARGS_SKETCH, *ext_param])
        ...
        if pad:
            ...
            vec = np.concatenate([vec, CAD_EOS_VEC[np.newaxis].repeat(pad_len, axis=0)], axis=0)
        return vec
...
class CADSequence(object):
    ...
    def from_vector(vec, is_numerical=False, n=256):
        commands = vec[:, 0]
        ext_indices = [-1] + np.where(commands == CAD_EXT_IDX)[0].tolist()
        ext_seq = []
    ...
    def to_vector(self, max_n_ext=10, max_n_loops=6, max_len_loop=15, max_total_len=60, pad=False):
        ...
        vec_seq = np.concatenate([vec_seq, CAD_EOS_VEC[np.newaxis]], axis=0)

        # add EOS padding
        if pad and vec_seq.shape[0] < max_total_len:
            pad_len = max_total_len - vec_seq.shape[0]
            vec_seq = np.concatenate([vec_seq, CAD_EOS_VEC[np.newaxis].repeat(pad_len, axis=0)], axis=0)
    ...
    def normalize(self, size=1.0):
        ...
        scale = size * CAD_NORM_FACTOR / np.max(np.abs(self.bbox))
