...
def create_CAD(cad_seq: CADSequence):
    ...
    for extrude_op in cad_seq.seq[1:]:
        ...
        if extrude_op.operation == CAD_EXTRUDE_OPERATIONS.index("NewBodyFeatureOperation") or \
                extrude_op.operation == CAD_EXTRUDE_OPERATIONS.index("JoinFeatureOperation"):
            body = BRepAlgoAPI_Fuse(body, new_body).Shape()
        elif extrude_op.operation == CAD_EXTRUDE_OPERATIONS.index("CutFeatureOperation"):
            body = BRepAlgoAPI_Cut(body, new_body).Shape()
        elif extrude_op.operation == CAD_EXTRUDE_OPERATIONS.index("IntersectFeatureOperation"):
...
def create_by_extrude(extrude_op: Extrude):
    ...
    if extrude_op.extent_type == CAD_EXTENT_TYPE.index("SymmetricFeatureExtentType"):
        body_sym = BRepPrimAPI_MakePrism(face, ext_vec.Reversed()).Shape()
        body = BRepAlgoAPI_Fuse(body, body_sym).Shape()
    if extrude_op.extent_type == CAD_EXTENT_TYPE.index("TwoSidesFeatureExtentType"):
