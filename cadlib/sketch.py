...
class SketchBase(object):
    ...
    def normalize(self, size=256):
        ...
        scale = (size / 2 * CAD_NORM_FACTOR - 1) / cur_size # prevent potential overflow if data augmentation applied
    ...
    def denormalize(self, bbox_size, size=256):
        ...
        scale = bbox_size / (size / 2 * CAD_NORM_FACTOR - 1)
...
class Loop(SketchBase):
    ...
    def to_vector(self, max_len=None, add_sol=True, add_eos=True):
        ...
        if add_sol:
            loop_vec = np.concatenate([CAD_SOL_VEC[np.newaxis], loop_vec], axis=0)
        if add_eos:
            loop_vec = np.concatenate([loop_vec, CAD_EOS_VEC[np.newaxis]], axis=0)
        if max_len is None:
            return loop_vec
        ...
        if loop_vec.shape[0] > max_len:
            ...
            pad_vec = np.tile(CAD_EOS_VEC, max_len - loop_vec.shape[0]).reshape((-1, len(CAD_EOS_VEC)))
...
class Profile(SketchBase):
    ...
    def from_vector(vec, start_point=None, is_numerical=True):
        all_loops = []
        command = vec[:, 0]
        end_idx = command.tolist().index(CAD_EOS_IDX)
        indices = np.where(command[:end_idx] == CAD_SOL_IDX)[0].tolist() + [end_idx]
        for i in range(len(indices) - 1):
            loop_vec = vec[indices[i]:indices[i + 1]]
            loop_vec = np.concatenate([loop_vec, CAD_EOS_VEC[np.newaxis]], axis=0)
            if loop_vec[0][0] == CAD_SOL_IDX and loop_vec[1][0] not in [CAD_SOL_IDX, CAD_EOS_IDX]:
    ...
    def to_vector(self, max_n_loops=None, max_len_loop=None, pad=True):
        ...
        profile_vec = np.concatenate([profile_vec, CAD_EOS_VEC[np.newaxis]], axis=0)
        if pad:
            pad_len = max_n_loops * max_len_loop - profile_vec.shape[0]
            profile_vec = np.concatenate([profile_vec, CAD_EOS_VEC[np.newaxis].repeat(pad_len, axis=0)], axis=0)
        return profile_vec
