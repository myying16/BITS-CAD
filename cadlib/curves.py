...
class Line(CurveBase):
    ...
    def to_vector(self):
        vec = [CAD_LINE_IDX, self.end_point[0], self.end_point[1]]
        return np.array(vec + [PAD_VAL] * (1 + CAD_N_ARGS - len(vec)))
...
class Arc(CurveBase):
    ...
    def to_vector(self):
        ...
        return np.array([CAD_ARC_IDX, self.end_point[0], self.end_point[1], sweep_angle, int(self.clock_sign), PAD_VAL,
                         *[PAD_VAL] * CAD_N_ARGS_EXT])
...
class Circle(CurveBase):
    ...
    def to_vector(self):
        vec = [CAD_CIRCLE_IDX, self.center[0], self.center[1], PAD_VAL, PAD_VAL, self.radius]
        return np.array(vec + [PAD_VAL] * (1 + CAD_N_ARGS - len(vec)))
