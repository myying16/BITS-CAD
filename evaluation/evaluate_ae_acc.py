...
each_cmd_cnt = np.zeros((len(CAD_COMMANDS),))
...
for name in tqdm(filenames):
    ...
    for j in range(len(gt_cmd)):
        ...
        if cmd in [CAD_SOL_IDX, CAD_EOS_IDX]:
            continue
...
for i in range(each_param_acc.shape[0]):
    print(CAD_COMMANDS[i] + " param acc:", each_param_acc[i][args_mask[i].astype(np.bool)], file=fp)
fp.close()
