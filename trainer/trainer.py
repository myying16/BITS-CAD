...
class TrainerED(BaseTrainer):
    ...
    def forward(self, data):
        cad_data = data['cad']  #command(256,60)，args(256,60,16)
        cad_cmd = cad_data['command'].cuda()  # ([256, 60])
        cad_args = cad_data['args'].cuda()  # ([256, 60])
        svg_data = data['svg']
        svg_command = svg_data['command'].cuda() #256,100
        svg_args = svg_data['args'].cuda() #256,100,8
        text=data["text"]
        outputs = self.net(text, svg_command, svg_args)
    ...
    def logits2vec(self, outputs, refill_pad=True, to_numpy=True):
        ...
        out_command = torch.argmax(torch.softmax(outputs['command_logits'], dim=-1), dim=-1)  # (N, S=60)
        out_args = torch.argmax(torch.softmax(outputs['args_logits'], dim=-1), dim=-1) - 1  # (N, S, N_ARGS=16)
        if refill_pad: # fill all unused element to -1
            mask = ~torch.tensor(CAD_CMD_ARGS_MASK).bool().cuda()[out_command.long()]
            out_args[mask] = -1

        out_cad_vec = torch.cat([out_command.unsqueeze(-1), out_args], dim=-1)  #(B, 60, 17)
        if to_numpy:
