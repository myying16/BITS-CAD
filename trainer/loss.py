...
class NewCADLoss(nn.Module):
    ...
    def forward(self, outputs, cad_data):
        # Target
        tgt_commands = cad_data["command"].cuda()  #([256, 60])
        tgt_args = cad_data["args"].cuda()  #([256, 60, 16])

        visibility_mask = _get_visibility_mask(tgt_commands, seq_dim=-1)
        padding_mask = _get_padding_mask_cad(tgt_commands, seq_dim=-1, extended=True) * visibility_mask.unsqueeze(-1) #([256, 60])

        # Prediction
        command_logits = outputs["command_logits"]  #dict ([256, 60, 6])
...
def gumbel_loss(pred, target, mask, tolerance=3, alpha=2.0):
    B, S, N_ARGS, N_CLASS = pred.shape  #([256, 60, 16, 257])
    target += 1
